...
class SMPLRenderer(object):
    def __init__(self,
        ...
        with open("/home/guanghan/code/SMPLify_TF/smpl/models/bodyparts.pkl", 'rb') as f:
            v_ids = pickle.load(f)
        if Utility.render_fingers is False:
        ...
        if Utility.render_toes is False:
            hands = np.concatenate((v_ids['toes_r'], v_ids['toes_l']))
            self.faces = np.array(filter(lambda face: np.intersect1d(face, hands).size == 0, self.faces))
        if Utility.render_fingers is False and Utility.render_toes is False:
            hands = np.concatenate((v_ids['fingers_r'], v_ids['fingers_l'],
                                    v_ids['toes_r'], v_ids['toes_l']))
        if Utility.render_hands is False:
