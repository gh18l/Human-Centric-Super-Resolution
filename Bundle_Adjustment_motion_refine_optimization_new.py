...
def refine_optimization(poses, betas, trans, data_dict, hmr_dict, LR_cameras, texture_img, texture_vt, Util):
    ...
    param_pose_full1 = tf.concat([param_rots, param_poses], axis=1)  ## N * 72
    #objs['Prior_Loss'] = Util.Prior_Loss_refine * tf.reduce_sum(tf.square(param_poses - poses[:frame_num, 3:72]))
    objs['Prior_Loss'] = Util.Prior_Loss_refine * tf.reduce_sum(tf.square(param_pose_full1 - poses[:frame_num]))
...
def main():
    ...
    Bundle_Adjustment_motion_refine_optimization(params)
