...
def Bundle_Adjustment_optimization(params):
    ...
    param_rots = tf.Variable(hmr_thetas[:, :3].reshape([-1, 3]), dtype=tf.float32)
    param_poses = tf.Variable(hmr_thetas[:, 3:72].reshape([-1, 69]), dtype=tf.float32)
    ...
    objs['J2D_Loss'] = Util.J2D_Loss * tf.reduce_sum(weights * tf.reduce_sum(tf.square(j2ds_body_est - j2ds), 1))
    ...
    with tf.Session() as sess:
        ...
        optimizer = scipy_pt(loss=loss,
                             ...
                             options={'eps': 1e-20, 'ftol': 1e-20, 'maxiter': 10000, 'disp': True})
