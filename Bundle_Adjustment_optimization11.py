...
def Bundle_Adjustment_optimization(params):
    ...
    for ind_batch in range(Util.batch_num):
        ...
        j3ds_body = j3ds[ind_start:ind_end, 2:, :]
        j3ds_head = j3ds[ind_start:ind_end, 14:16, :]
        j3ds_foot = j3ds[ind_start:ind_end, :2, :]
        j3ds_face = j3dsplus[ind_start:ind_end, 14:19, :]
        ...
        objs["angle_elbow_knee"] = 0.08 * tf.reduce_sum(w1[0] *
            tf.exp(param_poses[ind_start:ind_end, 52]) + w1[1] *
            tf.exp(-param_poses[ind_start:ind_end, 55]) + w1[2] *
            tf.exp(-param_poses[ind_start:ind_end, 9]) + w1[3] *
            tf.exp(-param_poses[ind_start:ind_end, 12]))
